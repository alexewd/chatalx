...
class LogAnalyzer(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        self.table.setMinimumHeight(400)  # Увеличиваем минимальную высоту таблицы
        self.layout.addWidget(self.table, stretch=1)  # Растягиваем таблицу
        ...
        try:
            ...
            with open(log_file_path, 'r') as f:
                for line in f:
                    ...
                    if match:
                        host, ip, un2, datetime, request, status, post_sa_data = match.groups()
                        row_data = [host, ip, un2, datetime, request, status, post_sa_data]
                        rows.append(row_data)

            # Создаем DataFrame
            df_log = pd.DataFrame(rows, columns=['HOST', 'IP', 'UN2', 'date_time', 'request', 'SA', 'post_sa_data'])
    ...
    def extract_user_agent(self, post_sa_data):
        ...
        if len(matches) >= 2:  # Берем вторую пару кавычек (User-Agent)
            return matches[1]
        elif len(matches) == 1:  # Если только одна пара, возвращаем её
            return matches[0]
        return ""  # Если ничего нет, возвращаем пустую строку
    ...
    def update_table(self, df):
        ...
        # Растягиваем столбцы для удобства
