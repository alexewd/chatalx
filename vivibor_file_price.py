...
class App(QWidget):
    def __init__(self):
        ...
        # Кнопка очистки
        self.btnClear = QPushButton('Очистить таблицу', self)
    ...
    def load_file(self):
        ...
        if file_name:
            try:
                ...
                QMessageBox.information(self, "Отладка", f"Загруженные данные:\n{self.mb.head()}")  # Отображение первых 5 строк
                QMessageBox.information(self, "Отладка", f"Столбцы: {self.mb.columns.tolist()}")  # Вывод названий столбцов
            except Exception as e:
                QMessageBox.critical(self, "Ошибка", f"Не удалось загрузить файл: {e}")

def search_df(self):
    QMessageBox.information(self, "Отладка", "Метод search_df вызван")  # Отладочное сообщение
    if self.mb is None:
    ...
    try:
        ...
        if self.mb.empty:
            QMessageBox.warning(self, "Предупреждение", "Загруженный файл не содержит данных!")
            return
        ...
        QMessageBox.critical(self, "Ошибка", f"Ошибка в search_df: {e}")
