...
# Явный путь для выходного файла, отключаем буферизацию
output_file = r'c:\bac_c\nekropol\burials.txt'
logging.info(f"Открытие файла для записи: {output_file}")

# Открываем файл без буферизации (buffering=0)
print("Запуск обработки всего файла...")
logging.info("Запуск обработки всего файла...")
with open(output_file, 'w', encoding='utf-8', buffering=1) as f:  # buffering=1 для построчной записи
    for i in tqdm(range(total_pages), desc="Обработка страниц"):
        ...
        for line in text_lines:
            ...
            if line.strip().startswith(fio):
                ...
                logging.info(f"Попытка записи: ФИО: {fio}, Год рождения: {birth_year}, Год смерти: {death_year}")
                ...
                print(f"Записано: {fio}, {birth_year}-{death_year}")
                fio_idx += 1
