...
results = []
output_file = r'c:\bac_c\nekropol\burials_x_i.txt'
batch_size = 20  # Записываем каждые 20 страниц

# Проверка доступности директории
output_dir = os.path.dirname(output_file)
if not os.path.exists(output_dir):
    logging.error(f"Директория {output_dir} не существует!")
    raise FileNotFoundError(f"Директория {output_dir} не существует!")
...
logging.info(f"Путь для записи файла: {output_file}")
...
for i in tqdm(range(total_pages), desc="Обработка страниц"):
    ...
    if (i + 1) % batch_size == 0 or (i + 1) == total_pages:
        logging.info(f"Подготовка к сохранению порции после страницы {i+1}")
...
if results:
    logging.info("Сохранение остатка результатов")
    save_batch(results, output_file)
...
df.to_csv(r'c:\bac_c\nekropol\burials_x_i.csv', index=False, encoding='utf-8')
logging.info("Результаты сохранены в burials_x_i.csv")
df.to_excel(r'c:\bac_c\nekropol\burials.xlsx', index=False)
logging.info("Результаты сохранены в burials_x_i.xlsx")
