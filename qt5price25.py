...
# Игнорирование предупреждений о депрекации
    ...
    def load_file(self):
        ...
        if file_name:
            try:
                ...
                # Проверка на NaN
                if self.mb.isnull().values.any():
                    QMessageBox.warning(self, "Предупреждение", "Файл содержит недопустимые значения (NaN).")
                    return
    ...
    def search_df(self):
        ...
        try:
            ...
            if not search_result.empty:
                self.table.setRowCount(len(search_result))  # Установка количества строк в таблице
...
if __name__ == '__main__':
    app = QApplication(sys.argv)  # Исправлено: создание экземпляра QApplication
    window = App()
    window.show()
    sys.exit(app.exec_())
