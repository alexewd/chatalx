...
class LogAnalyzer(QtWidgets.QWidget):
    ...
    def process_logs(self):
        ...
        try:
            ...
            # Извлечение размера ответа и времени обработки
            df_log['UN3'] = df_log['UN3'].astype(str)  # Преобразуем в строку
            response_data = df_log['UN3'].str.split(' ', expand=True)

            # Проверяем, что у нас есть два столбца после разделения
            if response_data.shape[1] >= 2:
                df_log['response_size'] = pd.to_numeric(response_data[0], errors='coerce')
                df_log['processing_time'] = pd.to_numeric(response_data[1].str.split(':').str[0], errors='coerce')
            else:
                df_log['response_size'] = pd.NA
                df_log['processing_time'] = pd.NA
